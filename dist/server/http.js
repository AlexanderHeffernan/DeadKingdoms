...
export function createHandler(world, clients) {
    return async function handler(req, res) {
        ...
        if (req.method === "POST" && url.pathname === "/api/join")
            return joinGame(req, res, world);
        if (req.method === "POST" && url.pathname === "/api/command")
            return receiveCommand(req, res, world);
        if (req.method === "POST" && url.pathname === "/api/leave")
            return leaveGame(req, res, world);
        if (req.method === "GET" && url.pathname === "/events")
            return streamEvents(req, res, world, clients, url);
        if (req.method === "GET" && url.pathname === "/api/snapshot")
            return json(res, makeSnapshot(world));
        if (req.method === "GET" && url.pathname === "/api/soundtrack")
            return listSoundtrack(res);
        if (req.method === "GET" && url.pathname.startsWith("/assets/soundtrack/"))
            return serveSoundtrack(req, res, url);
