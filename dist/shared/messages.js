...
export function makeSnapshot(world, playerId = null, sentExplored = null) {
    ...
    const filterVisible = (entities) => {
        ...
        for (const id in entities) {
            ...
            if (isVisible(visibleSet, entity.x, entity.y, entity.size || 1, world.map.size))
                out[id] = entity;
        }
        return out;
    };
    ...
    return {
        ...
        playerId,
        map: world.map,
        players: Object.fromEntries(Object.entries(world.players).map(([id, player]) => [
            id,
            {
                id,
                name: player.name,
...
function cachedVisibility(world, playerId) {
    ...
    if (!player)
        return null;
    if (player._visCache && player._visCache.tick === world.tick)
        return player._visCache;
