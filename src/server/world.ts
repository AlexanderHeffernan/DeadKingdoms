...
function gatherSeconds(resource: ResourceNode | Building): number {
  if (resource.kind === "building" && resource.type === "farm") return 8;
  return 1.1;
}

function gatherAmount(unit: Unit, resource: ResourceNode | Building): number {
  if (resource.kind === "building" && resource.type === "farm") return 6;
  return (UNIT_DEFS[unit.type] as typeof UNIT_DEFS["villager"]).carryCapacity;
}
